...
% Set the left side of the equation (tensor product Laplacian)
  A_1 = diag(2*ones(1,Nx-1)) + diag((-1)*ones(1,Nx-2),-1) + diag((-1)*ones(1,Nx-2),1);
  A_2 = diag(2*ones(1,Ny-1)) + diag((-1)*ones(1,Ny-2),-1) + diag((-1)*ones(1,Ny-2),1); 
% Set the the Neumann boundary conditions
  C1 = zeros(Ny - 1); 
  C2 = zeros(Ny - 1);
  C1(1,1) = 1;
  C2(Ny-1,Ny-1) = 1;
  matA = kron(A_2, eye(Nx-1)) + kron(eye(Ny-1), A_1) - kron(C1,eye(Nx-1)) - kron(C2,eye(Nx-1));

% Set the right side of the equation
  F_buttom = 20*h;
  F_right = 100;
  F_top = 40*h;
  F1 = kron([1; zeros(Ny-2,1)], ones(Nx - 1,1)*F_buttom) + kron([zeros(Ny-2,1); 1],ones(Nx - 1,1)*F_top) + kron(ones(Ny - 1,1)*F_right, [zeros(Nx-2,1);1]);
...
% Set the boundary conditions for the plot
  uu(Nx+1,1:Ny+1) = 100;
  uu(1,1:Ny+1) = 0;
  uu(2:Nx,1) = 20*h + uu(2:Nx,2); 
  uu(2:Nx,Ny+1) = 40*h + uu(2:Nx,Ny);
  [xx,yy] = meshgrid(x,y);
